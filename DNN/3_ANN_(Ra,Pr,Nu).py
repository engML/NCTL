...
data_train_all = np.log10(data_train_all)
...
data_train, data_val_2 = train_test_split(data_train_all, test_size=0.15, random_state=rnd_seed)
...
transfer_folder = tl_2 if revised else tl_1
...
if revised:
    ...
    Ym = layers.Concatenate(name = 'Nu_Ra__Nu_Pr__')([Y_1, Y_2])
    dense_7 = layers.Dense(16, activation='tanh', name='16_nodes_hidden_layer_7')(Ym)
    dense_8 = layers.Dense(16, activation='tanh', name='16_nodes_hidden_layer_8')(dense_7)
    outputs = layers.Dense(1, name='output_Nu_number')(dense_8)
    model = keras.Model(inputs=[model_1.input], outputs=outputs, name="Block_III")
